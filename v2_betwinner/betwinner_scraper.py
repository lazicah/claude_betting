"""
Betwinner Live Soccer Odds Scraper
=====================================
Fetches live soccer match data (scores + odds) from the Betwinner LineFeed API.

Betwinner runs on the same 1xSoftware/LineFeed platform as 1xBet, Melbet,
22Bet, and 888Starz — only the domain changes.

API endpoint (live soccer):
    https://betwinner.com/LineFeed/Get1x2_VZip
    ?sports=1&count=50&lng=en&tf=800000&tz=3&mode=4&getEmpty=true

Each live match gets its own CSV file:
    {team1}_vs_{team2}_{tournament}_bwn_{date}.csv

Usage:
    python v2_betwinner/betwinner_scraper.py
    python v2_betwinner/betwinner_scraper.py --interval 5
    python v2_betwinner/betwinner_scraper.py -o my_data_dir

Note: Betwinner may be geo-restricted in some regions. A VPN may be required.
"""
...
log = logging.getLogger("betwinner")
...
class BetwinnerClient:
    """Thin wrapper around the Betwinner LineFeed API."""
...
def run(
    output_dir: str = "match_database/betwinner",
    interval: float = DEFAULT_POLL_INTERVAL,
) -> None:
    client = BetwinnerClient()
    writer = MatchCSVWriter(output_dir)

    log.info("Betwinner live SOCCER odds scraper started")
...
def main():
    parser = argparse.ArgumentParser(
        description="Betwinner live soccer odds scraper (LineFeed API)"
    )
    parser.add_argument("-o", "--output-dir", default="match_database/betwinner")
