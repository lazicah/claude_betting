"""
888Starz Live Soccer Odds Scraper
====================================
Fetches live soccer match data (scores + odds) from the 888Starz LineFeed API.

888Starz runs on the same 1xSoftware/LineFeed platform as 1xBet, Melbet,
22Bet, and Betwinner — only the domain changes.

API endpoint (live soccer):
    https://888starz.bet/LineFeed/Get1x2_VZip
    ?sports=1&count=50&lng=en&tf=800000&tz=3&mode=4&getEmpty=true

Fallback domain: 888starz.com (same API path).

Each live match gets its own CSV file:
    {team1}_vs_{team2}_{tournament}_888_{date}.csv

Usage:
    python v2_888starz/starz888_scraper.py
    python v2_888starz/starz888_scraper.py --interval 5
    python v2_888starz/starz888_scraper.py -o my_data_dir

Note: 888Starz may be geo-restricted in some regions. A VPN may be required.
"""
...
log = logging.getLogger("888starz")
...
class Starz888Client:
    """Thin wrapper around the 888Starz LineFeed API."""
...
def run(
    output_dir: str = "match_database/888starz",
    interval: float = DEFAULT_POLL_INTERVAL,
) -> None:
    client = Starz888Client()
    writer = MatchCSVWriter(output_dir)

    log.info("888Starz live SOCCER odds scraper started")
...
def main():
    parser = argparse.ArgumentParser(
        description="888Starz live soccer odds scraper (LineFeed API)"
    )
    parser.add_argument("-o", "--output-dir", default="match_database/888starz")
