"""
Melbet Live Soccer Odds Scraper
=================================
Fetches live soccer match data (scores + odds) from the Melbet LineFeed API.

Melbet runs on the same 1xSoftware/LineFeed platform as 1xBet, 22Bet,
Betwinner, and 888Starz — only the domain changes.

API endpoint (live soccer):
    https://melbet.com/LineFeed/Get1x2_VZip
    ?sports=1&count=50&lng=en&tf=800000&tz=3&mode=4&getEmpty=true

Each live match gets its own CSV file:
    {team1}_vs_{team2}_{tournament}_mlb_{date}.csv

Usage:
    python v2_melbet/melbet_scraper.py
    python v2_melbet/melbet_scraper.py --interval 5
    python v2_melbet/melbet_scraper.py -o my_data_dir

Note: Melbet may be geo-restricted in some regions. A VPN may be required.
"""
...
class MelbetClient:
    """Thin wrapper around the Melbet LineFeed API."""
...
def run(
    output_dir: str = "match_database/melbet",
    interval: float = DEFAULT_POLL_INTERVAL,
) -> None:
    client = MelbetClient()
    writer = MatchCSVWriter(output_dir)

    log.info("Melbet live SOCCER odds scraper started")
...
def main():
    parser = argparse.ArgumentParser(
        description="Melbet live soccer odds scraper (LineFeed API)"
    )
    parser.add_argument("-o", "--output-dir", default="match_database/melbet")
