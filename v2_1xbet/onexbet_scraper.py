"""
1xBet Live Soccer Odds Scraper
================================
Fetches live soccer match data (scores + odds) from the 1xBet LineFeed API.

1xBet uses its own proprietary LineFeed platform, shared with Melbet, 22Bet,
Betwinner, and 888Starz (same 1xSoftware backend).

API endpoint (live soccer):
    https://1xbet.com/LineFeed/Get1x2_VZip
    ?sports=1&count=50&lng=en&tf=800000&tz=3&mode=4&getEmpty=true

Each live match gets its own CSV file:
    {team1}_vs_{team2}_{tournament}_1xb_{date}.csv

Usage:
    python v2_1xbet/onexbet_scraper.py
    python v2_1xbet/onexbet_scraper.py --interval 5
    python v2_1xbet/onexbet_scraper.py -o my_data_dir

Note: 1xBet is geo-restricted in some countries. A VPN may be required.
"""
...
# Config
# ---------------------------------------------------------------------------
API_BASE = "https://1xbet.com"
BOOKMAKER_TAG = "1xb"

SOCCER_SPORT_ID = 1          # 1xSoftware sport ID for soccer
LINEFEED_MODE_LIVE = 4       # mode=4 = in-play events

DEFAULT_POLL_INTERVAL = 5.0  # seconds (conservative — rate-limit friendly)

# Market type IDs used by the 1xSoftware LineFeed platform
MARKET_1X2 = 1     # Match result (home win / draw / away win)
MARKET_TOTAL = 17  # Total goals over/under

# Match period codes (PE field)
PERIOD_NAMES = {
    1: "",    # 1st half in progress — show only the minute
    2: "",    # 2nd half in progress
    3: "ET1", # Extra time 1st half
    4: "ET2", # Extra time 2nd half
    5: "PEN", # Penalty shoot-out
}
...
class OnexbetClient:
    """Thin wrapper around the 1xBet LineFeed API."""
    ...
    def fetch_live_soccer(self) -> dict:
        """Fetch all live soccer events with embedded odds."""
...
def _extract_score(event: dict) -> tuple[str, str]:
    """Return (home_score, away_score) from an event dict."""
    ss = event.get("SS", "")
...
def _extract_match_time(event: dict) -> str:
    """Return the match minute string from an event dict."""
    t = event.get("T")
    if t is not None:
        return str(t)
    sc = event.get("SC") or {}
...
def _extract_match_status(event: dict) -> str:
    """Return human-readable match status (HT, FT, PEN …) or empty."""
    period = event.get("PE")
...
def _parse_event(ev: dict, tournament: str) -> dict | None:
    """Parse a single 1xBet LineFeed event into our standard format."""
    team1 = ev.get("O1", "")
    team2 = ev.get("O2", "")
...
def parse_response(data: dict) -> List[dict]:
    """Parse the LineFeed API response into a list of event dicts.

    Handles both wrapped (championship → RL list) and flat formats.
    """
    events: List[dict] = []
...
def run(
    output_dir: str = "match_database/1xbet",
    interval: float = DEFAULT_POLL_INTERVAL,
) -> None:
    client = OnexbetClient()
    writer = MatchCSVWriter(output_dir)

    log.info("1xBet live SOCCER odds scraper started")
...
def main():
    parser = argparse.ArgumentParser(
        description="1xBet live soccer odds scraper (LineFeed API)"
    )
    parser.add_argument(
        "-o", "--output-dir",
        default="match_database/1xbet",
        help="Directory for CSV files (default: match_database/1xbet)",
    )
    parser.add_argument(
        "-i", "--interval",
        type=float,
